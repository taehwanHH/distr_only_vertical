...
def sensor_read(num_robot, num_sensor):
    ...
    return torch.tensor(touch_vector, dtype=torch.float32).view(num_robot,-1).to(DEVICE)
...
def actuator_write(num_robot, action):
    for i in range(num_robot):
        ...
        sim.data.ctrl[actuator_2_idx] = action[i][0]
        sim.data.ctrl[actuator_3_idx] = action[i][1]
...
class RoboticEnv:
    def __init__(self, Max_time=Max_time):
        ...
        self.num_robot = 4
        self.num_joint = 2
        self.state_dim = self.num_sensor_output
        self.action_dim = self.num_joint
    ...
    def reset(self):
        ...
        rand_ctrl = torch.rand(self.num_robot,self.action_dim)*Act_max
